...
//
#define OS_INT_DisableAll()                   \
  {                                           \
    __asm volatile("cpsid i" : : : "memory"); \
  }
#define OS_INT_EnableAll()                    \
  {                                           \
...
//
#define OS_INT_PreserveAll(p)                   \
  {                                             \
    __asm volatile("mrs  %[Para], primask \n\t" \
                   : [Para] "=l"(*(p))          \
                   :                            \
                   : "memory");                 \
  }
#define OS_INT_RestoreAll(p)                    \
  {                                             \
    __asm volatile("msr  primask, %[Para] \n\t" \
                   :                            \
                   : [Para] "l"(*(p))           \
                   : "memory");                 \
  }
...
#define OS_INT_Disable()                         \
  {                                              \
    __asm volatile("msr  basepri, %[IPL] \n\t"   \
                   :                             \
...
#define OS_INT_Preserve(p)                      \
  {                                             \
    __asm volatile("mrs  %[Para], basepri \n\t" \
                   : [Para] "=r"(*(p))          \
                   :                            \
                   : "memory");                 \
  }
#define OS_INT_Restore(p)                        \
  {                                              \
    __asm volatile("msr  basepri, %[Para]  \n\t" \
                   :                             \
                   : [Para] "r"(*(p))            \
                   : "memory");                  \
  }
...
#define TRACE_RECORD_TASK_INFO(pTask)               \
  if (OS_Global.pTraceAPI != NULL)                  \
  {                                                 \
    OS_Global.pTraceAPI->pfRecordTaskInfo((pTask)); \
  }
#define TRACE_RECORD_API_VOID(Id)                                  \
  if (OS_Global.pTraceAPI != NULL)                                 \
  {                                                                \
    OS_Global.pTraceAPI->pfRecordVoid((Id) + OS_TRACE_API_OFFSET); \
  }                                                                \
  if ((Id) <= 255u)                                                \
  {                                                                \
    OS_TRACE_Void((OS_U8)(Id));                                    \
  }
#define TRACE_RECORD_API_U32(Id, Para0)                                            \
  if (OS_Global.pTraceAPI != NULL)                                                 \
  {                                                                                \
    OS_Global.pTraceAPI->pfRecordU32((Id) + OS_TRACE_API_OFFSET, (OS_U32)(Para0)); \
  }                                                                                \
  if ((Id) <= 255u)                                                                \
  {                                                                                \
    OS_TRACE_Data((OS_U8)(Id), (int)(Para0));                                      \
  }
#define TRACE_RECORD_API_U32X2(Id, Para0, Para1)                                                      \
  if (OS_Global.pTraceAPI != NULL)                                                                    \
  {                                                                                                   \
    OS_Global.pTraceAPI->pfRecordU32x2((Id) + OS_TRACE_API_OFFSET, (OS_U32)(Para0), (OS_U32)(Para1)); \
  }                                                                                                   \
  if ((Id) <= 255u)                                                                                   \
  {                                                                                                   \
    OS_TRACE_Data((OS_U8)(Id), (int)(Para0));                                                         \
...
#define TRACE_RECORD_API_U32X3(Id, Para0, Para1, Para2)                                                                \
  if (OS_Global.pTraceAPI != NULL)                                                                                     \
  {                                                                                                                    \
    OS_Global.pTraceAPI->pfRecordU32x3((Id) + OS_TRACE_API_OFFSET, (OS_U32)(Para0), (OS_U32)(Para1), (OS_U32)(Para2)); \
  }                                                                                                                    \
  if ((Id) <= 255u)                                                                                                    \
  {                                                                                                                    \
    OS_TRACE_Data((OS_U8)(Id), (int)(Para0));                                                                          \
...
#define TRACE_RECORD_API_U32X4(Id, Para0, Para1, Para2, Para3)                                                                          \
  if (OS_Global.pTraceAPI != NULL)                                                                                                      \
  {                                                                                                                                     \
    OS_Global.pTraceAPI->pfRecordU32x4((Id) + OS_TRACE_API_OFFSET, (OS_U32)(Para0), (OS_U32)(Para1), (OS_U32)(Para2), (OS_U32)(Para3)); \
  }                                                                                                                                     \
  if ((Id) <= 255u)                                                                                                                     \
  {                                                                                                                                     \
    OS_TRACE_Data((OS_U8)(Id), (int)(Para0));                                                                                           \
...
#define TRACE_RECORD_API_PTR(Id, Para0)                                                                                             \
  if (OS_Global.pTraceAPI != NULL)                                                                                                  \
  {                                                                                                                                 \
    OS_Global.pTraceAPI->pfRecordU32((Id) + OS_TRACE_API_OFFSET, OS_Global.pTraceAPI->pfPtrToId((OS_U32)(OS_PTR_TO_VALUE(Para0)))); \
  }                                                                                                                                 \
  if ((Id) <= 255u)                                                                                                                 \
  {                                                                                                                                 \
    OS_TRACE_Ptr((OS_U8)(Id), (Para0));                                                                                             \
  }
#define TRACE_RECORD_API_PTR_U32(Id, Para0, Para1)                                                                                                     \
  if (OS_Global.pTraceAPI != NULL)                                                                                                                     \
  {                                                                                                                                                    \
    OS_Global.pTraceAPI->pfRecordU32x2((Id) + OS_TRACE_API_OFFSET, OS_Global.pTraceAPI->pfPtrToId((OS_U32)(OS_PTR_TO_VALUE(Para0))), (OS_U32)(Para1)); \
  }                                                                                                                                                    \
  if ((Id) <= 255u)                                                                                                                                    \
  {                                                                                                                                                    \
    OS_TRACE_PtrU32((OS_U8)(Id), (Para0), (OS_U32)(Para1));                                                                                            \
  }
#define TRACE_RECORD_API_PTR_U32X2(Id, Para0, Para1, Para2)                                                                                                             \
  if (OS_Global.pTraceAPI != NULL)                                                                                                                                      \
  {                                                                                                                                                                     \
    OS_Global.pTraceAPI->pfRecordU32x3((Id) + OS_TRACE_API_OFFSET, OS_Global.pTraceAPI->pfPtrToId((OS_U32)(OS_PTR_TO_VALUE(Para0))), (OS_U32)(Para1), (OS_U32)(Para2)); \
  }                                                                                                                                                                     \
  if ((Id) <= 255u)                                                                                                                                                     \
  {                                                                                                                                                                     \
    OS_TRACE_PtrU32((OS_U8)(Id), (Para0), (OS_U32)(Para1));                                                                                                             \
...
#define TRACE_RECORD_API_PTR_U32X3(Id, Para0, Para1, Para2, Para3)                                                                                                                       \
  if (OS_Global.pTraceAPI != NULL)                                                                                                                                                       \
  {                                                                                                                                                                                      \
    OS_Global.pTraceAPI->pfRecordU32x4((Id) + OS_TRACE_API_OFFSET, OS_Global.pTraceAPI->pfPtrToId((OS_U32)(OS_PTR_TO_VALUE(Para0))), (OS_U32)(Para1), (OS_U32)(Para2), (OS_U32)(Para3)); \
  }                                                                                                                                                                                      \
  if ((Id) <= 255u)                                                                                                                                                                      \
  {                                                                                                                                                                                      \
    OS_TRACE_PtrU32((OS_U8)(Id), (Para0), (OS_U32)(Para1));                                                                                                                              \
...
#define TRACE_RECORD_API_PTR_U32X4(Id, Para0, Para1, Para2, Para3, Para4)                                                                                                                                 \
  if (OS_Global.pTraceAPI != NULL)                                                                                                                                                                        \
  {                                                                                                                                                                                                       \
    OS_Global.pTraceAPI->pfRecordU32x5((Id) + OS_TRACE_API_OFFSET, OS_Global.pTraceAPI->pfPtrToId((OS_U32)(OS_PTR_TO_VALUE(Para0))), (OS_U32)(Para1), (OS_U32)(Para2), (OS_U32)(Para3), (OS_U32)(Para4)); \
  }                                                                                                                                                                                                       \
  if ((Id) <= 255u)                                                                                                                                                                                       \
  {                                                                                                                                                                                                       \
    OS_TRACE_PtrU32((OS_U8)(Id), (Para0), (OS_U32)(Para1));                                                                                                                                               \
...
#define TRACE_ON_ISR_ENTER()                 \
  if (OS_Global.pTraceAPI != NULL)           \
  {                                          \
    OS_Global.pTraceAPI->pfRecordEnterISR(); \
  }
#define TRACE_ON_ISR_EXIT()                 \
  if (OS_Global.pTraceAPI != NULL)          \
  {                                         \
    OS_Global.pTraceAPI->pfRecordExitISR(); \
  }
#define TRACE_ON_ISR_EXIT_TO_SCHEDULER()               \
  if (OS_Global.pTraceAPI != NULL)                     \
  {                                                    \
    OS_Global.pTraceAPI->pfRecordExitISRToScheduler(); \
  }
#define TRACE_ON_TASK_START_EXEC(TaskId)                                           \
  if (OS_Global.pTraceAPI != NULL)                                                 \
  {                                                                                \
    OS_Global.pTraceAPI->pfRecordTaskStartExec((OS_U32)OS_PTR_TO_VALUE((TaskId))); \
  };                                                                               \
  OS_TRACE_Void(OS_TRACE_ID_ACTIVATE)
#define TRACE_ON_TASK_STOP_EXEC()                \
  if (OS_Global.pTraceAPI != NULL)               \
  {                                              \
    OS_Global.pTraceAPI->pfRecordTaskStopExec(); \
  }
#define TRACE_ON_TASK_START_READY(TaskId)                                           \
  if (OS_Global.pTraceAPI != NULL)                                                  \
  {                                                                                 \
    OS_Global.pTraceAPI->pfRecordTaskStartReady((OS_U32)OS_PTR_TO_VALUE((TaskId))); \
  }
#define TRACE_ON_TASK_STOP_READY(TaskId, Para0)                                                           \
  if (OS_Global.pTraceAPI != NULL)                                                                        \
  {                                                                                                       \
    OS_Global.pTraceAPI->pfRecordTaskStopReady((OS_U32)(OS_PTR_TO_VALUE(TaskId)), (unsigned int)(Para0)); \
  }
#define TRACE_ON_TASK_CREATE(TaskId)                                            \
  if (OS_Global.pTraceAPI != NULL)                                              \
  {                                                                             \
    OS_Global.pTraceAPI->pfRecordTaskCreate((OS_U32)(OS_PTR_TO_VALUE(TaskId))); \
  }
#define TRACE_ON_IDLE()                  \
  if (OS_Global.pTraceAPI != NULL)       \
  {                                      \
    OS_Global.pTraceAPI->pfRecordIdle(); \
  }
#define TRACE_ON_TIMER_ENTER(TimerId)                                            \
  if (OS_Global.pTraceAPI != NULL)                                               \
  {                                                                              \
    OS_Global.pTraceAPI->pfRecordEnterTimer((OS_U32)(OS_PTR_TO_VALUE(TimerId))); \
  };                                                                             \
  OS_TRACE_Ptr(OS_TRACE_ID_TIMERCALLBACK, TimerId)
#define TRACE_ON_TIMER_EXIT()                 \
  if (OS_Global.pTraceAPI != NULL)            \
  {                                           \
    OS_Global.pTraceAPI->pfRecordExitTimer(); \
  }
#if (OS_SUPPORT_TRACE_API_END != 0)
#define TRACE_RECORD_API_END(Id)                                      \
  if (OS_Global.pTraceAPI != NULL)                                    \
  {                                                                   \
    OS_Global.pTraceAPI->pfRecordEndCall((Id) + OS_TRACE_API_OFFSET); \
  }
#define TRACE_RECORD_API_END_U32(Id, Para0)                                               \
  if (OS_Global.pTraceAPI != NULL)                                                        \
  {                                                                                       \
    OS_Global.pTraceAPI->pfRecordEndCallU32((Id) + OS_TRACE_API_OFFSET, (OS_U32)(Para0)); \
  }
#else
#define TRACE_RECORD_API_END(Id)
#define TRACE_RECORD_API_END_U32(Id, Para0)
...
#define TRACE_ON_TASK_TERMINATED(TaskId)                          \
  if (OS_Global.pTraceAPI != NULL)                                \
  {                                                               \
    OS_Global.pTraceAPI->pfRecordTaskTerminate((OS_U32)(TaskId)); \
  }
#define TRACE_RECORD_OBJNAME(Id, Para0)                          \
  if (OS_Global.pTraceAPI != NULL)                               \
  {                                                              \
    OS_Global.pTraceAPI->pfRecordObjName((OS_U32)(Id), (Para0)); \
  }
...
#define TRACE_RECORD_API_PTR_U32(Id, Para0, Para1)          \
  if ((Id) <= 255u)                                         \
  {                                                         \
    OS_TRACE_U32Ptr((OS_U8)(Id), (OS_U32)(Para1), (Para0)); \
  }
#define TRACE_RECORD_API_PTR_U32X2(Id, Para0, Para1, Para2) \
  if ((Id) <= 255u)                                         \
  {                                                         \
...
#define OS_MPU_PRIVSTATE_ENTER()         \
  {                                      \
    OS_U32 State;                        \
    State = OS_MPU_GetPrivilegedState(); \
    OS_MPU_EnterPrivilegedState();
#endif

#ifndef OS_MPU_PRIVSTATE_LEAVE
#define OS_MPU_PRIVSTATE_LEAVE()      \
  OS_MPU_LeavePrivilegedState(State); \
  }
#endif
...
#define OS_INT_DecRI()                                                   \
  {                                                                      \
    OS_ASSERT_DICNT();                                                   \
    OS_Global.Counters.Cnt.DI = (OS_U8)(OS_Global.Counters.Cnt.DI - 1u); \
    if (OS_Global.Counters.Cnt.DI == 0u)                                 \
    { /*lint !e548 No else required*/                                    \
      OS_INT_Enable();                                                   \
    }                                                                    \
  }

#define OS_INT_EnableConditional()       \
  {                                      \
    OS_ASSERT_DICNT();                   \
    if (OS_Global.Counters.Cnt.DI == 0u) \
    { /*lint !e548 No else required*/    \
...
#define OS_INT_PreserveAndDisable(p) \
  {                                  \
    OS_INT_Preserve(p);              \
    OS_INT_Disable();                \
  }
...
#define OS_MARK_OUTOF_ISR()      \
  {                              \
    if (OS_Global.InInt == 0u)   \
    {                            \
      OS_Error(OS_ERR_LEAVEINT); \
    }                            \
    OS_Global.InInt--;           \
  }
